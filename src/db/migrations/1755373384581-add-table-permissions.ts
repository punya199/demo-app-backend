...
export class AddTablePermissions1755373384581 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TYPE "public"."permissions_feature_name_enum" AS ENUM('house_rent', 'bill', 'user')`
    )
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE INDEX "IDX_5f6890b717390a248d7d3efcf8" ON "permissions" ("user_id") WHERE deleted_at IS NULL`
    )
    await queryRunner.query(
      `CREATE INDEX "IDX_dad2c96aedb953d5645e52eeac" ON "permissions" ("feature_name") WHERE deleted_at IS NULL`
    )
    await queryRunner.query(
      `CREATE UNIQUE INDEX "IDX_b4532c45d93ae8d17fde779a30" ON "permissions" ("user_id", "feature_name") WHERE deleted_at IS NULL`
    )
    await queryRunner.query(
      `ALTER TABLE "permissions" ADD CONSTRAINT "FK_03f05d2567b1421a6f294d69f45" FOREIGN KEY ("user_id") REFERENCES "users"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`
    )
  }
